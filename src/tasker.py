...
def main(): 
    ...
    rqPreProccess = Queue('preproc', connection=redisLink)
    workQLow = deque()
    workQHigh = deque()
    workQPre = deque()
    threadKeeper = {}
    ...
    while True:
        ## check for files
        fwReturn = files.FindWork(workQLow, workQHigh, threadKeeper, taskerLogger)
        if fwReturn == 0 and len(threadKeeper) == 0:
            taskerLogger.info("No files; UTE sleeping")
            while files.FindWork(workQLow, workQHigh, threadKeeper, taskerLogger) == 0:
                sleep(20)
